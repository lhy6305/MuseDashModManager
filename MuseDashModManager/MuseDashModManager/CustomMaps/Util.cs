...
namespace MuseDashModManager.CustomMaps
{
    class Util
    {
        ...
        public static void Init()
        {
            ...
            // 加载谱面
            DirectoryInfo di = new DirectoryInfo(Global.AssetsDirectory);
        ...
        public static CustomMap LoadCustomMapFromDirectory(DirectoryInfo di)
        {
            ...
            cm.Music = n[0];
            // 分配一个UID，取文件夹名去掉空格			
			// 新版uid必须是xx-xx的形式，其中xx为十进制数字（不一定几位）
        ...
        public static BMSAndStage LoadAndCreateStageInfo(string filename) // 不需要.bms后缀名
        {
            /* 1.加载bms
             * 2.转换为MusicData
             * 3.创建StageInfo
