...
namespace MuseDashModManager.CustomMaps
{
    class Patch
    {
        ...
        {
            if (__instance.name == "albumFancyScrollView")
            {
                // 创建自定义谱面的Album按键
        ...
        {
            // 禁用自定义谱面的成绩上传
            if (Singleton<DataManager>.instance["Account"]["SelectedMusicUid"].GetResult<string>().StartsWith("9999-")) return false;
            return true;
        }
